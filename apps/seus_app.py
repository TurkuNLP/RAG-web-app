...
seus_app = Flask(__name__, template_folder="../templates", static_folder="../static")
...
global DATA_PATH
load_config('seus')
from parameters import EMBEDDING_MODEL, LLM_MODEL, PROMPT_TEMPLATE, DATA_PATH, REPHRASING_PROMPT, STANDALONE_PROMPT, ROUTER_DECISION_PROMPT
...
@seus_app.route("/")
def index():
    return render_template('seus_index.html', root=root)
...
@seus_app.route('/documents/<document_name>', methods=['GET'])
...
@seus_app.route('/update-settings', methods=['POST'])
def update_settings():
    data = request.get_json()
...
@seus_app.route('/clear_chat_history', methods=['POST'])

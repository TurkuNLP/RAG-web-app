...
@news_app.route("/")
def index():
    return render_template('news_index.html', root=root)


def init_app():
    load_rag()
    news_app.config['UPLOAD_FOLDER'] = DATA_PATH
...
@news_app.route('/files/<filename>', methods=['GET'])
def serve_file(filename):
    return send_from_directory(news_app.config['UPLOAD_FOLDER'], filename)


@news_app.route("/get", methods=["POST"])
def chat():
...
@news_app.route('/update-settings', methods=['POST'])
def update_settings():
    data = request.get_json()
...
@news_app.route('/clear_chat_history', methods=['POST'])
...
if __name__ == '__main__':
    init_app()
    news_app.run(port=6669,debug=False)